    ...
    <div class="rounded-2xl sm:rounded-3xl border border-white/10 bg-gradient-to-b from-white/[0.04] to-transparent p-5 sm:p-8 lg:p-12 grid lg:grid-cols-2 gap-8 lg:gap-12 items-start">
        ...
        <div>
            ...
            <p class="mt-4 text-white/50 text-sm sm:text-base leading-relaxed">
                Testez notre réceptionniste IA en temps réel. Posez-lui une question ou demandez un rendez-vous pour voir comment il répond !
            </p>
            ...
            <div class="mt-6 flex flex-wrap gap-3">
                ...
                <button class="rounded-lg border border-white/10 text-white/50 text-sm font-medium px-4 py-2.5 flex items-center gap-2 hover:bg-white/5 transition">
            ...
            <p class="mt-6 text-xs text-white/30 flex items-center gap-2">
                ...
                Ceci est un mode test. Aucune donnée n'est enregistrée.
            </p>
        </div>
        ...
        <div x-data="{
                messages: [
                    { from: 'bot', text: 'Bonjour ! 👋 Je suis votre réceptionniste IA. Comment puis-je vous aider aujourd’hui ?' }
                ],
                draft: '',
                send(text) {
                    ...
                    setTimeout(() => {
                        this.messages.push({ from: 'bot', text: 'Avec plaisir ! Pouvez-vous me préciser votre demande pour que je vous oriente au mieux ? 😊' });
                        this.$nextTick(() => this.$refs.scrollArea.scrollTop = this.$refs.scrollArea.scrollHeight);
                    }, 700);
                }
            }"
            ...
            <div class="bg-gradient-to-r from-sky-500 to-blue-600 px-4 sm:px-5 py-3.5 sm:py-4 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    <img src="{{ asset('logo.png') }}" alt="IADial" class="h-8 w-8 rounded-full bg-white/20 p-1">
                    <div>
                        <p class="text-sm font-semibold text-white">Réceptionniste IA</p>
                        <p class="text-xs text-white/80 flex items-center gap-1">
                            <span class="w-1.5 h-1.5 rounded-full bg-emerald-300"></span> En ligne
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="border-t border-white/10 p-2.5 sm:p-3 flex items-center gap-2">
                <input x-model="draft" @keydown.enter="send()" type="text" placeholder="Écrivez votre message..."
                    class="flex-1 min-w-0 bg-white/5 border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white placeholder-white/30 focus:outline-none focus:border-sky-400/50">
                <button @click="send()" class="w-10 h-10 shrink-0 rounded-lg bg-gradient-to-r from-sky-400 to-indigo-500 flex items-center justify-center text-black">
